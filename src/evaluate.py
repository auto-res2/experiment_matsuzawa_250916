...
def create_plots(exp_name, all_seeds_data, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{exp_name}_accuracy_vs_time.pdf'))
    plt.close()
    ...
    if 'predictor_corr_rho' in log_df.columns:
        ...
        plt.legend(title='Model')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'{exp_name}_rho_vs_epoch.pdf'))
        plt.close()
...
def main(config):
    ...
    for exp_name, exp_config in config['experiments'].items():
        ...
        exp_results_dir = os.path.join(base_dir, f'results_{exp_name}')
        ...
        # Create plots
        create_plots(exp_name, all_seeds_data, exp_results_dir)
    ...
    # Final JSON output
    output_path = os.path.join(base_dir, 'final_summary.json')
    ...
    # Save as a single parseable JSON for stdout
