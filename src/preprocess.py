...
def acquire_hipaa_medtriage_data(config: dict) -> pd.DataFrame:
    ...
    if not mimic_path.is_dir():
        raise FileNotFoundError(f"MIMIC-IV directory not found at '{mimic_path}'. NO-FALLBACK: This dataset is required for the specified experiment.")
    ...
    if not mimic_file.exists():
        raise FileNotFoundError(f"Processed MIMIC-IV file 'discharge_summaries.parquet' not found in '{mimic_path}'. NO-FALLBACK.")
...
def acquire_hf_dataset(dataset_name: str, config_name: str, split: str, column: str, task: str) -> pd.DataFrame:
    ...
    try:
        ...
        dataset = load_dataset(dataset_name, config_name, split=split, token=hf_token, download_config=DownloadConfig(use_etag=False))
        df = dataset.to_pandas()
        df = df[[column]].rename(columns={column: 'prompt'})
        df['task'] = task
        return df
    except Exception as e:
        raise ConnectionError(f"Failed to download '{dataset_name}' from Hugging Face Hub. NO-FALLBACK. Error: {e}")
...
def run(config: dict):
    ...
    for task in tasks:
        if task == 'HIPAA-MedTriage-5':
            ...
            df_list.append(acquire_hf_dataset('super_natural_instructions', None, 'train[:2000]', 'definition', 'Super-NI'))
        elif task == 'GSM8K':
            df_list.append(acquire_hf_dataset('openai/gsm8k', 'main', 'train', 'question', 'GSM8K'))
        else:
