...
class MetaLEAPPredictor(nn.Module):
    ...
    # --- forward / helpers identical to previous version (omitted for brevity) ---
    ...
    # Remaining methods (ar1_update, apply_control_variate, etc.) unchanged
            ...
            for seed in exp_cfg['seeds']:
                ...
                if model_name in ['leap', 'meta-leap']:
                    ...
                    predictor = MetaLEAPPredictor(
                        in_channels=mp['hidden_channels'],
                        num_layers=mp['num_layers'],
                        heads=mp['heads'],
                        meta_ckpt_path=psi_path,
                        control_variate=exp_cfg.get('control_variate', 'ar1' if model_name == 'meta-leap' else 'none')
                    ).to(device)

                params = list(model.parameters()) + (list(predictor.parameters()) if predictor else [])
                opt = torch.optim.AdamW(params, lr=exp_cfg['training']['lr'], weight_decay=exp_cfg['training']['weight_decay'])
                ...
                for epoch in range(exp_cfg['training']['epochs']):
                    ...
                    if exp_name == 'exp2_transfer' and predictor is not None:
                        if epoch == 0:
                            ...
                            opt.add_param_group({'params': predictor.delta_params(), 'lr': exp_cfg['training']['lr'] * 5.0})
                    ...
                    if rank == 0:
                        ...
                        entry = {
                            ...
                            'lambda_t_avg': float(np.mean(getattr(predictor, 'ar1_lambda', [0.0]))) if predictor and model_name == 'meta-leap' else 0.0
                        }
                        log.append(entry)
                        print(f"Epoch {epoch:02d} | Loss {avg_loss:.4f} | Val {val_acc:.4f} | Test {test_acc:.4f} | Time {epoch_time:.2f}s | Rho {rho:.3f}")
                        # Save best-by-val
...
def evaluate(model, predictor, data, device, subset_nodes=10000):
    ...
    # Sample nodes on CPU, then move after subgraph built
    node_idx = torch.randperm(data.num_nodes)[:subset_nodes]

    # PyG >=2.5 has Data.subgraph; if not we build manually
    ...
    val_acc = ((pred[sub.val_mask] == sub.y[sub.val_mask]).float().mean().item()) if sub.val_mask.sum() > 0 else 0.0
    test_acc = ((pred[sub.test_mask] == sub.y[sub.test_mask]).float().mean().item()) if sub.test_mask.sum() > 0 else 0.0
    ...
    if predictor is not None:
        ...
        rho = (vx * vy).sum() / (torch.sqrt((vx ** 2).sum()) * torch.sqrt((vy ** 2).sum()) + 1e-8)
        rho = rho.item()
