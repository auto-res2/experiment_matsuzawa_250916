...
"""
    try:
        response_text = query_openai_api(
            scorer_model, grading_prompt, temperature=0.0, max_tokens=100
        )
    ...
    def record_call(self, is_bootstrap=False):
        ...
        if is_bootstrap:
            if self.bootstrap_calls >= self.bootstrap_budget:
                raise RuntimeError(
                    f"Bootstrap API budget of {self.bootstrap_budget} exceeded."
                )
...
class ReflectBO:
    ...
    def bootstrap(self, initial_prompts):
        ...
        for prompt in seed_prompts:
            ...
            paraphrases = self.paraphraser_pipe(
                prompt, num_return_sequences=5, max_length=128, num_beams=5
            )
            ...
            for para in paraphrases:
                ...
                similarity = np.dot(original_embedding, para_embedding.T) / (
                    np.linalg.norm(original_embedding) * np.linalg.norm(para_embedding)
                )
                if similarity >= 0.86:
        ...
        if contrastive_pairs["positive"] or contrastive_pairs["negative"]:
            ...
            optimizer = optim.Adam(
                ...
                lr=self.config["hyperparameters"]["learning_rate"][0],
            )
            ...
            for _ in range(3):  # 3 epochs
                for anchor, positive in contrastive_pairs["positive"]:
                    ...
                    loss = -torch.nn.functional.cosine_similarity(
                        torch.from_numpy(anchor_emb).to(self.device),
                        torch.from_numpy(positive_emb).to(self.device),
                        dim=0,
                    )
                    loss.backward()
                    optimizer.step()
        ...
        if self.use_attribution and len(self.attribution_maps) > 0:
            ...
            for c in candidates:
                ...
                padded_attn = torch.zeros(
                    1, attn.shape[0], attn.shape[1], 2048, 2048
                )
            ...
            acq_scores -= np.array(risk_scores)
    ...
    def update(self, eval_result):
        ...
        if self.use_attribution and len(self.attribution_maps) > 1:
            ...
            for i, att_map in enumerate(self.attribution_maps):
                ...
                padded_attn = torch.zeros(
                    1, att_map.shape[0], att_map.shape[1], 2048, 2048
                )
...
def run_single_trial(config, method, seed, data, device):
    ...
    budget_tracker = BudgetTracker(
        config["optimization_params"]["api_budget"],
        config["optimization_params"]["bootstrap_budget"],
    )
    ...
    while budget_tracker.can_continue():
        if method == "Random":
            ...
            answer = query_openai_api(
                config["model_names"]["generator"], prompt
            )
            scores = grade_with_gpt4(
                config["model_names"]["scorer"], prompt, answer
            )
            latency = time.time() - start_time
            eval_result = {
                "prompt": prompt,
        ...
        cvar = (
            np.mean(sorted(optimizer.y_c)[-int(len(optimizer.y_c) * 0.1) :])
            if (optimizer and len(optimizer.y_c) > 10)
            else 0.0
        )
        step_metrics = {
        ...
        print(
            f"Step {step_metrics['step']}: H={step_metrics['helpfulness']:.3f}, C={step_metrics['severity_cost']:.3f}"
        )
...
def run(config):
    ...
    if not data:
        raise FileNotFoundError(
            f"No processed data found in {data_path}. Please run preprocess stage."
        )
    ...
    tracker = EmissionsTracker(
        ...
        project_name=f"run_{config['name']}",
        log_level="error",
    )
    tracker.start()

    exp_config = config[f"experiment_{config['experiment_id']}"]
    methods = exp_config.get(
