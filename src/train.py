...
class ReflectBOOptimizer(BaseOptimizer):
    ...
    def embed(self, texts):
        ...
        with torch.no_grad():
            embeddings = (
                self.encoder(**inputs, output_hidden_states=True, return_dict=True)
    ...
    def propose(self):
        ...
        scores = self.cvar_ei_acquisition(
            mu,
            std,
    ...
    def update(self, prompt, result):
        ...
        target = torch.tensor([result["severity_cost"]], dtype=torch.float32).to(
            self.device
        )
        self.risk_cnn.train()
...
def run_experiment_2(config, data, device):
    ...
    for method in config["experiment_2"]["methods"]:
        ...
        global_encoder = AutoModel.from_pretrained(config["model_names"]["encoder"]).to(
            device
        )
        ...
        for episode in range(n_episodes):
            ...
            for i in range(n_clients):
                ...
                if method in {"REFLECT-BO", "REFLECT-BO-NoShift"}:
                    ...
                    encoder_optimizer = optim.Adam(
                        client.optimizer.encoder.parameters(),
